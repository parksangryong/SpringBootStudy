...
import com.spring.cart.demo.api.dto.ErrorRes;
...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<ErrorRes> respond(HttpStatus status, String code, String message) {
        return ResponseEntity.status(status)
                .body(new ErrorRes(message, status.value(), code));
    }

    // 도메인 예외: 우리가 정의한 코드 그대로
    @ExceptionHandler(AppException.class)
    public ResponseEntity<ErrorRes> handleApp(AppException ex) {
        var e = ex.error();
    ...
    public ResponseEntity<ErrorRes> handleRse(ResponseStatusException ex) {
    ...
    public ResponseEntity<ErrorRes> handleNotReadable(HttpMessageNotReadableException ex) {
        return respond(HttpStatus.BAD_REQUEST, "BAD_REQUEST", "Malformed request");
    }

    // 400: @Valid 바디 검증
    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<ErrorRes> handleMethodArgumentNotValid(MethodArgumentNotValidException ex) {
    ...
    public ResponseEntity<ErrorRes> handleNoHandler(NoHandlerFoundException ex) {
